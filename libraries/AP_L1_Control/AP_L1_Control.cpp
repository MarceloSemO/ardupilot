...
void AP_L1_Control::update_loiter_3d(const struct Location &S2center, const Vector3f &ercv, int32_t S2radius, const float theta_rho, int8_t orientation, struct Location &aircraft_loc, Vector3f &aircraft_vel, struct Location &desired_loc)
{
    ...
    const float sin_theta_rho = sinf(radians(theta_rho));
    // distance between the center of the sphere and the center of the circle
    const int32_t D = S2radius * cos_theta_rho;
    // radius of the circle
    const int32_t S1radius = S2radius * sin_theta_rho;
    ...
    // of lateral projection of circle (this has a form of an ellipse).
    ...
    Vector3f s1ctoav(S1center.get_distance_NED(_current_loc));
